...
### 【블록 3】 두괄식 판정 근거 — 5줄 요약 (고정 순서)

번호 매긴 **5줄** 리스트. 각 줄은 정량 수치 또는 등재 여부 등 **팩트 기반**이어야 함. LLM 도구(`generate_report1`)·`parseReport1Payload`에서 **줄별 min/max 길이**를 검증한다.

| 줄 | 카테고리 | 길이 | 비고 |
|----|----------|------|------|
| 1 | 시장 수요·의료환경 (인구·보건·CSS + 표적 prevalence) | 30~**200**자 | prevalence·출처 **풀 인용** 가능 |
| 2 | 규제 | 30~100자 | |
| 3 | 무역 | 30~100자 | |
| 4 | 조달 | 30~100자 | |
| 5 | 유통 (4개 파트너 전체명 + AHP·PSI) | 30~**250**자 | |

**Aceclofenac 전용 (`scope=latam_average`)**  
- prevalence 문자열에 지역 평균 스코프가 포함된 경우: **1번 줄 본문**에서는 `(…scope=latam_average…)` 괄호를 넣지 않는다.  
- 대신 `block3_latam_scope_footnote`에 고정 각주 한 줄을 두고, 웹/PDF에서 판정 근거 표 **바로 아래**에 표시한다.  
- **그 외 7개 INN**은 각주 필드 없음(빈 문자열).

**작성 원칙**:
- 각 줄에 최소 1개 정량 수치(%, $, 날짜) 또는 등재 여부 포함(도구 minLength 30)
