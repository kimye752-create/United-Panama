...
const SOURCE_NAME = "cphi_japan";
const DATA_PATH = "scripts/data/partners_cphi.json";

const CphiRowSchema = z.object({
...
type CphiRow = z.infer<typeof CphiRowSchema>;

async function main(): Promise<void> {
  const rows = parseSeedJson<CphiRow>(DATA_PATH, CphiRowSchema).map((row) => ({
...
void main().catch((error: unknown) => {
  ...
  process.stderr.write(`[seed_partners_cphi] ${message}\n`);
  process.exit(1);
});
