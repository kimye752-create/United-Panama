...
export function buildFallbackReport(input: FallbackInput): Report1Payload {
  ...
  const prevLine = input.prevalenceMetric
    ...
    : "파나마 1인당 보건지출 $1,547 + CSS 가입률 70%로 의료 접근성 안정적";

  const reasoningRaw: string[] = [
    `시장·의료: 인구 451만, 1인당 보건지출 $1,547, CSS 70%; ${prevLine}.`,
  ...
  const pricingRaw = `공공 낙찰가: PanamaCompra 최근 낙찰 ${String(input.panamacompraCount)}건. 민간 소매가: Arrocha·Metro Plus 데이터 수집 중(Phase 2 크롤링 완료 후 보강). CABAMED 가격 통제 리스트 적용 여부는 ACODECO Resolución No. 174 기준 추가 확인 필요. 경쟁사 포지셔닝: 현지 제조사 Medipan·Rigar 및 다국적 임포터(GSK 등) 대비 차별화 포인트는 한국 GMP 기반 품질 + FTA 관세 0% 가격 경쟁력.`;
